...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{data }\OtherTok{\textless{}{-}} \FunctionTok{merge}\NormalTok{(data, HCL\_behavioural) }\CommentTok{\# merges with the common variables pNum and trial}

\NormalTok{data }\OtherTok{\textless{}{-}} 
  \FunctionTok{conditional\_transform}\NormalTok{(data, }
...
(\citeproc{ref-salvucci2000}{2000}){]}. Since points of fixation occur
when the eye is not in consistent motion, the algorithm computes the
...
algorithm then applys a dispersion check to ensure that the eye
maintains a relatively stable position across this period. If this
dispersion threshold is exceeded, then a new fixation is determined.
Fixations must be of a minimum length for classification (by default 150
ms).
...
\NormalTok{AOI\_areas }\OtherTok{\textless{}{-}} \FunctionTok{create\_AOI\_df}\NormalTok{()}
