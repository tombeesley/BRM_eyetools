...
Missing values in the x and y columns of the raw data should be expressed as "NA". An optional 5th column can be included, with encodes the participant ID, which can be numeric or character type:

-   pID = an index of the current participant in the data

For many of the functions in *eyetools* the *participant_ID* column allows the user to run the function across data from multiple participants. This functionality means that the user can avoid having to generate additional programming steps to analyse and combine the data from multiple participants
...
# perform a transormation of the data across the x coordinate midline
...
The eyetools package has a number of visualisations that allow for functional plots of the data, with minimal effort. All plots use the dominant graphical R package `ggplot`, which means that the resulting plots are ggplot objects and can be customised using the full suite of options for ggplot and its extensions.

`plot_spatial()` offers a simple means to view the data produced by *eyetools*. By default the plotting commands will plot all of the data that is passed to the function. However, trial and participant numbers can be specified. Here we plot the raw data from a single trial of one participant, with the detected fixations overlaid with the detected fixations. When using fixation data, the fixations are labelled in their temporal order (by default), enabling a clear presentation of how the fixations arose.
...
fixations <- fixations[fixations$pNum == 118,]
saccades <- saccades[saccades$pNum == 118,]
...
In addition to eye data, an image can be added to the plot, which is useful for inspecting data over a representation of the experimental task. If AOIs have been defined, these can be plotted as well. Here we demonstrate the plotting of the saccades, AOIs, and a background image:
...
plot_spatial(sac_data = saccades,
             AOIs = HCL_AOIs,
             trial_number = 6,
             bg_image = "images/HCL_sample_image.jpg")
...
AOI_areas[3,] <- c(960, 840, 200, 400) # Right circular AOI
