...
`plot_spatial()` offers a simple means to view raw data, fixation data, and saccades. In addition to eye data, an image can be added to the plot, which is useful for inspecting data over a representation of the experimental task. If AOIs have been defined, these can be plotted as well. By default the function will plot all of the data it is passed, but a trial number can be supplied to constrain the presentation of data.

```{r}

plot_spatial(fix_data = fixations,
             sac_data = saccades,
             trial_number = 3,
